...
class DyConv2D(nn.Module):
    ...
    def __init__(self,
        ...
        if dense:
            ...
            self.conv = SparseConv2d(
                            ...
                            base=base)
        if self.gn_inside:
            self.gn = nn.GroupNorm(num_groups=32, num_channels=out_channels, eps=1e-05)### 分组归一化，通道数分成按32个为一组，不改变输入张量的shape,只是分组normalization
    ...
    def forward(self, inputs_meta):
        ...
        if self.dense:
            ...
            if self.gn_inside:### 4层sp运算为true，最后的reg和cls为false
                if self.training:
                    out, mse = self.conv(inputs, mask, pw, gn=self.gn)
                    out = out + pw_back
                    return out, mse#### mse后面如何处理？未在代码里看到
                else:
