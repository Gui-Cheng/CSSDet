...
class AnchorDYHead(BaseDenseHead):
    ...
    def loss_by_feat(
            ...
            active_positions,### 对应的train 代码return的值
            total_positions,
            mse_losses,
            batch_gt_instances: InstanceList,
            batch_img_metas: List[dict],
...
##### 稀疏损失###
        ...
        loss_cost = sum(loss_block)/len(loss_block)### mask损失，用来监督生成掩膜的

        loss_gns = []
        for a_loss_mse in mse_losses:
            loss_gns.append(sum(a_loss_mse)/len(a_loss_mse))
        loss_gn = sum(loss_gns)/len(loss_gns) * self.beta###CESC归一化损失
##### 稀疏损失####
