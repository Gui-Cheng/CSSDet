...
class SparseConv2d(torch.nn.modules.conv._ConvNd):
    ...
    def _slow_forward(self, input, hard, pw, weight, bias, gn):
        hard = hard.hard###0,1 mask [B,1,h,w]
        x = F.conv2d(input, weight, bias, self.stride,
                    self.padding, self.dilation, self.groups)   #### 卷积运算
        if gn != None:
            with torch.no_grad():
                x_total = gn(x)#### 分32个通道为一组，归一化
                x_total = hard * x_total#### 掩膜处理, 全局归一化后进行掩摸

            assert self.base == 0
            x_part = sparse_gn(x, gn, pw)#### 稀疏归一化后，进行掩摸，与pw有关，原理没看懂，大概是基于pw的值进行归一化
            x_part = hard * x_part
