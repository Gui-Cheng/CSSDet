...
e
        ...
        cls_mask = self.retina_cls_mask(cls_feat)### Adaptive multi-layer masking(AMM),基于Gumbel()函数生成的0，1，mask
        reg_mask = self.retina_reg_mask(reg_feat)### Adaptive multi-layer masking(AMM)
        ...
        for cls_conv in self.cls_convs:
            cls_feat, loss = cls_conv((cls_feat, cls_mask, cls_pws))## CESE,,DyConv2D，mse_loss
            cls_feat = self.a_relu(cls_feat)
            mask = cls_mask.hard
            if self.return_hards:
                cls_hards.append(mask)
            if self.training:
                active_positions.append(torch.sum(mask))### mask难道不是一样的么，打印一下
                total_positions.append(mask.numel())### 返回数组中元素的个数
                mse_losses.append(loss)

        for reg_conv in self.reg_convs:
            reg_feat, loss = reg_conv((reg_feat, reg_mask, reg_pws))## 稀疏卷积
        ...
        cls_score = self.retina_cls((cls_feat, cls_mask, None))### 稀疏卷积，无pw，gn_inside=false，先对cls_feat，进行卷积，再用mask掩膜，这也叫稀疏卷积？
        ...
        if self.training:
            ...
            return cls_score, bbox_pred, active_positions, total_positions, mse_losses #### anchor_dy_head.py，修改了计算loss函数
        else:
            if self.return_hards:
